 ...
 */
public class ClothesCoordination {
    public static void main(String[] args) {
        ClothesCoordination coordination = new ClothesCoordination();

        String[][] clothes1 = {{"yellow_hat", "headgear"}, {"blue_sunglasses", "eyewear"}, {"green_turban", "headgear"}};
        int output1 = 5;
        int result1 = coordination.solution(clothes1);
        Assertions.assertEquals(output1, result1);

        String[][] clothes2 = {{"crow_mask", "face"}, {"blue_sunglasses", "face"}, {"smoky_makeup", "face"}};
        int output2 = 3;
        int result2 = coordination.solution(clothes2);
    ...
    public int solution(String[][] clothes) {
        ...
        // 코디 조합의 경우의 수 : O(N) - 카테고리가 전부 다른 경우 최악
        for(int count : map.values()) {
            answer *= (count + 1); // 입는 경우의 수와 안입는(+1) 경우
        }

        return answer - 1;  // 아예 아무런 코디도 하지 않은 경우 제외
    }
}
